...
//   name: z.string().trim().nonempty("Name is required").min(3, 'MIN 3 CHARACTERS').max(50, 'Invalid Name').regex(/^[A-Za-z ]+$/, 'Invalid Name'),
//   studentNumber: z.string().trim().nonempty("Required")
//     .refine(val => /^25[0-9]{5,6}$/.test(val), "Invalid Student Number"),
//   email: z.string().trim().nonempty("Required").toLowerCase().email("Invalid Email").endsWith("@akgec.ac.in", "Invalid Email").regex(/^[a-z.]{3,}[0-9]+@akgec\.ac\.in$/, "Invalid Email"),
//   gender: z.enum(genders, { required_error: 'Select gender' }),
//   branch: z.enum(branches, { required_error: 'Select branch' }),
//   phone: z.string().trim().nonempty("Required").regex(/^[6-9]\d{9}$/, "Invalid Phone Number"),
...
import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
import { Lock } from 'lucide-react'; // Removed Mail import
                      ...
                      <AnimatePresence mode="wait">
                        {!isSuccess ? (
                          ...
                          >
                             ...
                             {/* --- START OF CLOSED MESSAGE (Styled) --- */}
                             <div className="flex flex-col items-center justify-center w-full h-full text-center">
                                ...
                                <motion.div 
                                  ...
                                  className="w-20 h-20 rounded-full bg-violet-500/10 border border-violet-500/30 flex items-center justify-center mb-6 shadow-[0_0_40px_rgba(139,92,246,0.2)]"
                                >
                                  <Lock className="w-10 h-10 text-violet-400" />
                                </motion.div>

                                <div className="mb-6">
                                  <h2 className="text-3xl lg:text-4xl font-bold text-white tracking-widest uppercase drop-shadow-2xl mb-2">
                                    Registrations Closed
                                  </h2>
                                  <p className="text-[#a78bfa] text-lg lg:text-xl font-medium tracking-widest uppercase opacity-80">
                                    NIMBUS 3.0
                                  </p>
                                </div>

                                <p className="text-gray-400 text-center text-sm lg:text-base max-w-sm leading-relaxed mb-8">
                                  The official registration window has concluded. Thank you for the overwhelming response!
                                </p>
                             </div>
                             {/* --- END OF CLOSED MESSAGE --- */}


                             {/* --- YOUR COMMENTED CODE START --- */}
                             {/* <div className="text-center lg:mb-3 mb-3 lg:m-0 flex-shrink-0">
                                <h2 className="text-xl lg:text-2xl font-bold text-white tracking-wide drop-shadow-2xl">REGISTRATION FORM</h2>
                                <p className="text-[#a78bfa] text-sm lg:text-lg font-medium tracking-widest uppercase opacity-80">NIMBUS 3.0</p>
                              </div>
